...
impl MetadataBlob {
    ...
    pub(crate) fn read_inodes(&mut self) -> Result<Vec<Inode>> {
        read_one_from_slice(&self.mmapped_region[..])
    }

    pub(crate) fn max_ino(&mut self) -> Result<Option<Ino>> {
...
impl Digest {
    pub(crate) fn new(digest: &[u8; SHA256_BLOCK_SIZE]) -> Self {
        Self(*digest)
    }
...
impl fmt::Display for Digest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut hex_string =
            Vec::from_iter_fallible(encode_hex_iter(&self.0)).map_err(|_| fmt::Error)?;
